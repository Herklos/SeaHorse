...
export function ToggleCard({ icon, title, subtitle, enabled, onToggle, disabled = false }: ToggleCardProps) {
  ...
  return (
    <Pressable
      ...
      className={`bg-white dark:bg-gray-900 rounded-2xl p-4 mb-2 border border-gray-100 dark:border-gray-800 flex-row items-center ${
        disabled ? "opacity-50" : "active:opacity-80"
      }`}
    >
      {icon}
      <View className="ml-3 flex-1">
        <Text className="text-base text-gray-900 dark:text-white font-medium">{title}</Text>
        {subtitle != null && (
      ...
      <View
        ...
        style={{ backgroundColor: enabled ? colors.primary : "#D1D5DB" }}
      >
        <View
